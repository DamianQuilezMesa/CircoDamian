...
/**
 * Gestión de espectáculos (CU5A).
 * El flujo tiene dos pasos:
 *   Paso 1: datos del espectáculo (nombre, fechas, coordinador).
 *   Paso 2: asignar números con su orden (mínimo 3).
 */
@Controller
public class GestionEspectaculoController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        cbEspectaculo.getSelectionModel().selectedItemProperty().addListener(
                (obs, viejo, nuevo) -> {
                    if (nuevo != null) cargarEspectaculo(nuevo);
                });
    ...
    private void guardar(ActionEvent e) {
        try {
            ...
            new Alert(Alert.AlertType.INFORMATION,
                    "Espectáculo '" + guardado.getNombre() + "' guardado con "
                    + asignados.size() + " números.", ButtonType.OK).showAndWait();
            reset();
        } catch (IllegalArgumentException ex) {
            error(ex.getMessage());
        }
    ...
    private void eliminarEspectaculo(ActionEvent e) {
        ...
        if (resp.isPresent() && resp.get() == ButtonType.YES) {
            try {
                ...
                new Alert(Alert.AlertType.INFORMATION,
                        "Espectáculo eliminado.", ButtonType.OK).showAndWait();
                reset();
            } catch (IllegalArgumentException ex) {
                error(ex.getMessage());
            }
    ...
    private void configurarCombos() {
        ...
        cbCoordinador.setConverter(new StringConverter<Coordinacion>() {
            ...
            @Override
            public Coordinacion fromString(String s) { return null; }
        });
        ...
        cbNumero.setConverter(new StringConverter<Numero>() {
            ...
            public String toString(Numero n) {
                ...
                if (n.getArtistas().isEmpty()) {
                    ...
                    arts = n.getArtistas().stream().map(Artista::getNombre).sorted()
    ...
    private void configurarTabla() {
        ...
        colArtistas.setCellValueFactory(d -> {
            ...
            String texto = arts.stream().map(Artista::getNombre).sorted()
                    .collect(Collectors.joining(", "));
