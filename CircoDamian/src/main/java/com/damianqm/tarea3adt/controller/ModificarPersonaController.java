...
public class ModificarPersonaController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        cbPersona.getSelectionModel().selectedItemProperty().addListener(
                (obs, viejo, nueva) -> {
                    if (nueva != null) cargarDatosPersona(nueva);
                });

        chkSenior.selectedProperty().addListener(
                (obs, viejo, nuevo) -> dpFechaSenior.setDisable(!nuevo));
        ...
        try {
            ...
            if (credOpt.isPresent()) {
                ...
                if (perfil == Perfil.ARTISTA) {
                    ...
                    personaService.modificarDatosArtista(
                            seleccionada.getId(),
                    ...
                    personaService.modificarDatosCoordinacion(
                            seleccionada.getId(), chkSenior.isSelected(), fecha);
                }
            }
    ...
    private void mostrarAyuda(ActionEvent e) {
        ...
        a.setContentText(
            "1. Selecciona la persona en el desplegable.\n" +
            "2. Edita los campos.\n" +
