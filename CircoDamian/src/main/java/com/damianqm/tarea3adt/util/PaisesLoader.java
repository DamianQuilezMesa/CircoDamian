...
/**
 * Carga los países desde /resources/paises.xml. Guarda un Map<códigoISO,
 * nombre> ordenado alfabéticamente por nombre.
 */
@Component
public class PaisesLoader {
	...
	private Map<String, String> cargarDesdeXml() {
		...
		try (InputStream is = getClass().getResourceAsStream("/paises.xml")) {
			...
			resultado.entrySet().stream().sorted(Map.Entry.comparingByValue())
