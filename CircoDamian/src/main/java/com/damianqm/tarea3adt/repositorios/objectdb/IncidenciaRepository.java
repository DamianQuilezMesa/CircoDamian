...
public class IncidenciaRepository {
    ...
    public IncidenciaRepository(@Value("${objectdb.url}") String url,
                                @Value("${objectdb.username}") String username,
    ...
    // CU9 – Resolver incidencia: marca como resuelta y persiste la resolución (transaccional)
    ...
    public List<Incidencia> consultar(TipoIncidencia tipo, Boolean resuelta,
        ...
        try {
            ...
            if (tipo != null)          jpql.append(" AND i.tipo = :tipo");
            if (resuelta != null)      jpql.append(" AND i.resuelta = :resuelta");
            if (idEspectaculo != null) jpql.append(" AND i.idEspectaculo = :idEsp");
            if (idNumero != null)      jpql.append(" AND i.idNumero = :idNum");
            if (desde != null)         jpql.append(" AND i.fechaHora >= :desde");
            if (hasta != null)         jpql.append(" AND i.fechaHora <= :hasta");
            ...
            if (resuelta != null)      query.setParameter("resuelta", resuelta);
            if (idEspectaculo != null) query.setParameter("idEsp",    idEspectaculo);
            if (idNumero != null)      query.setParameter("idNum",    idNumero);
    ...
    public List<Incidencia> findAll() {
        ...
        try {
            return em.createQuery(
                    "SELECT i FROM Incidencia i ORDER BY i.fechaHora DESC", Incidencia.class)
                    .getResultList();
        } finally {
            em.close();
        }
